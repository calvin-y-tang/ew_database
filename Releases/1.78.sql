...
--Update view to use new Web User StatusID instead of Active field
DROP VIEW vwClientWebAcct
GO
CREATE VIEW vwClientWebAcct
AS
    SELECT  dbo.tblCompany.intname AS company ,
            dbo.tblClient.lastname ,
            dbo.tblClient.firstname ,
            dbo.tblWebUser.UserID ,
            dbo.tblWebUser.Password ,
            dbo.tblWebUser.LastLoginDate ,
            dbo.tblWebUser.UserType ,
            dbo.tblWebUserStatus.Description
    FROM    dbo.tblWebUser
			INNER JOIN tblWebUserStatus ON tblWebUser.StatusID = tblWebUserStatus.StatusID
            INNER JOIN dbo.tblClient ON dbo.tblWebUser.WebUserID = dbo.tblClient.WebUserID
            INNER JOIN dbo.tblCompany ON dbo.tblClient.companycode = dbo.tblCompany.companycode
    WHERE   ( dbo.tblWebUser.UserType = 'CL' )
            AND dbo.tblWebUser.StatusID <> 2
    UNION
    SELECT  'Doctor/Provider' AS Company ,
            dbo.tblDoctor.lastname ,
            dbo.tblDoctor.firstname ,
            dbo.tblWebUser.UserID ,
            dbo.tblWebUser.Password ,
            dbo.tblWebUser.LastLoginDate ,
            dbo.tblWebUser.UserType ,
    ...
    FROM    dbo.tblWebUser
			INNER JOIN tblWebUserStatus ON tblWebUser.StatusID = tblWebUserStatus.StatusID
            INNER JOIN dbo.tblDoctor ON dbo.tblWebUser.WebUserID = dbo.tblDoctor.WebUserID
    WHERE   ( dbo.tblWebUser.UserType = 'DR' )
            AND dbo.tblWebUser.StatusID <> 2
    UNION
    SELECT  dbo.tblDoctor.companyname AS Company ,
            dbo.tblDoctor.lastname ,
            dbo.tblDoctor.firstname ,
            dbo.tblWebUser.UserID ,
            dbo.tblWebUser.Password ,
    ...
    FROM    dbo.tblWebUser
			INNER JOIN tblWebUserStatus ON tblWebUser.StatusID = tblWebUserStatus.StatusID
            INNER JOIN dbo.tblDoctor ON dbo.tblWebUser.WebUserID = dbo.tblDoctor.WebUserID
    WHERE   ( dbo.tblWebUser.UserType = 'OP' )
            AND dbo.tblWebUser.StatusID <> 2
    UNION
    SELECT  dbo.tblCCAddress.company ,
            dbo.tblCCAddress.lastname ,
            dbo.tblCCAddress.firstname ,
            dbo.tblWebUser.UserID ,
            dbo.tblWebUser.Password ,
            dbo.tblWebUser.LastLoginDate ,
            dbo.tblWebUser.UserType ,
    ...
    FROM    dbo.tblWebUser
			INNER JOIN tblWebUserStatus ON tblWebUser.StatusID = tblWebUserStatus.StatusID
            INNER JOIN dbo.tblCCAddress ON dbo.tblWebUser.WebUserID = dbo.tblCCAddress.WebUserID
    WHERE   ( ( dbo.tblWebUser.UserType = 'AT' )
              OR ( dbo.tblWebUser.UserType = 'CC' )
            )
            AND dbo.tblWebUser.StatusID <> 2
    UNION
    SELECT  dbo.tblTranscription.transcompany AS company ,
            '' AS lastname ,
            '' AS firstname ,
            dbo.tblWebUser.UserID ,
            dbo.tblWebUser.Password ,
    ...
    FROM    dbo.tblWebUser
			INNER JOIN tblWebUserStatus ON tblWebUser.StatusID = tblWebUserStatus.StatusID
            INNER JOIN dbo.tblTranscription ON dbo.tblWebUser.WebUserID = dbo.tblTranscription.WebUserID
    WHERE   ( dbo.tblWebUser.UserType = 'TR' )
            AND dbo.tblWebUser.StatusID <> 2

GO


--New Treating Physician Credentials field
ALTER TABLE [tblExaminee]
  ADD [TreatingPhysicianCredentials] VARCHAR(50)
GO
...
AS
    SELECT  dbo.tblCase.casenbr ,
            ...
            ISNULL(dbo.tblUser.lastname, '')
            + CASE WHEN ISNULL(tblUser.LastName, '') = ''
                        OR ISNULL(tblUser.FirstName, '') = '' THEN ''
                   ELSE ', '
              END + ISNULL(dbo.tblUser.firstname, '') AS scheduler ,
            ...
            dbo.tblExaminee.TreatingPhysicianCredentials AS TreatingPhysicianDegree
    FROM    dbo.tblCase
            ...
            LEFT OUTER JOIN dbo.tblEWFacilityLocal ON dbo.tblCase.EWReferralEWFacilityID = dbo.tblEWFacilityLocal.EWFacilityID
GO
...
SELECT     dbo.tblCase.casenbr, dbo.tblCase.claimnbr, dbo.tblExaminee.addr1 AS examineeaddr1, dbo.tblExaminee.addr2 AS examineeaddr2, 
                      ...
                      dbo.tblCase.ForecastDate, dbo.tblDoctor.PrintOnCheckAs, dbo.tblExaminee.TreatingPhysicianCredentials AS TreatingPhysicianDegree
...
--New table to track software usage
CREATE TABLE [dbo].[tblLogUsage]
    (
      [LogUsageID] [int] NOT NULL IDENTITY(1, 1) ,
...
GO
ALTER TABLE [dbo].[tblLogUsage] ADD PRIMARY KEY CLUSTERED  ([LogUsageID])
GO



UPDATE tblControl SET DBVersion='1.78'
GO
