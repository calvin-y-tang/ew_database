...
GO
/*
The type for column Int1 in table [dbo].[tblTask] is currently  DATETIME NULL but is being changed to  INT NULL. Data loss could occur.

The type for column Int2 in table [dbo].[tblTask] is currently  DATETIME NULL but is being changed to  INT NULL. Data loss could occur.
*/
GO
...
CREATE TABLE [dbo].[tmp_ms_xx_tblTask] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_tblTask1] PRIMARY KEY CLUSTERED ([TaskID] ASC)
);
...
IF EXISTS (SELECT TOP 1 1 
    ...
    BEGIN
        ...
        INSERT INTO [dbo].[tmp_ms_xx_tblTask] ([TaskID], [DateAdded], [UserIDAdded], [DateCompleted], [StatusMessge], [ProcessName], [TableType], [TableKey], [Date1], [Date2], [Int1], [Int2])
        SELECT   [TaskID],
                 [DateAdded],
                 [UserIDAdded],
                 [DateCompleted],
...
EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_tblTask1]', N'PK_tblTask', N'OBJECT';
...
GO
PRINT N'Creating [dbo].[tblCaseDocLog]...';
...
GO
PRINT N'Creating [dbo].[tblCaseDocLog].[IX_tblCaseDocLog_DocSeqNoEntity]...';
...
GO
PRINT N'Creating [dbo].[tblCaseDocuments].[IX_tblCaseDocDocuments_FolderID]...';


GO
CREATE NONCLUSTERED INDEX [IX_tblCaseDocDocuments_FolderID]
    ON [dbo].[tblCaseDocuments]([FolderID] ASC);
