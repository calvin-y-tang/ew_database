-- Update Database to ver. 1.06. Generated on 4/29/2010
ALTER TABLE [tblDoctor]
  ADD [Booking] INTEGER DEFAULT ((2))
GO
...
CREATE VIEW [dbo].[vw30dayschedule]
AS
SELECT     TOP (100) PERCENT dbo.tblDoctorSchedule.doctorcode, dbo.tblDoctorSchedule.locationcode, dbo.tblLocation.location, dbo.tblDoctorSchedule.date, 
                      dbo.tblDoctorSchedule.status, dbo.tblDoctorSchedule.casenbr1, dbo.tblDoctorSchedule.casenbr2, dbo.tblDoctorSchedule.CaseNbr3, dbo.tblDoctor.Booking
FROM         dbo.tblDoctorSchedule INNER JOIN
                      dbo.tblDoctorLocation ON dbo.tblDoctorSchedule.doctorcode = dbo.tblDoctorLocation.doctorcode AND 
                      dbo.tblDoctorSchedule.locationcode = dbo.tblDoctorLocation.locationcode INNER JOIN
                      dbo.tblDoctor ON dbo.tblDoctorLocation.doctorcode = dbo.tblDoctor.doctorcode INNER JOIN
                      dbo.tblLocation ON dbo.tblDoctorLocation.locationcode = dbo.tblLocation.locationcode
WHERE     (dbo.tblDoctorSchedule.status <> 'Off')
ORDER BY dbo.tblDoctorSchedule.date
...
SELECT     TOP 100 PERCENT dbo.tblDoctorSchedule.locationcode, dbo.tblDoctorSchedule.date, dbo.tblDoctorSchedule.starttime, 
                      ...
                      tblimedata.fax,case when (select top 1 type from tblrecordhistory where casenbr = dbo.tblcase.casenbr) = 'F' then 'Films' else '' end as films
...
SELECT     TOP 100 PERCENT dbo.tblDoctorSchedule.locationcode, dbo.tblDoctorSchedule.date, dbo.tblDoctorSchedule.starttime, 
                      ...
                      END AS scheduledescription, tblservices.shortdesc, tblimedata.fax,case when (select top 1 type from tblrecordhistory where casenbr = tblcase.casenbr) = 'F' then 'Films' else '' end as films
...
AS
SELECT     TOP (100) PERCENT dbo.tblDoctor.lastname, dbo.tblDoctor.firstname, dbo.tblLocation.location, dbo.tblDoctorSchedule.date, dbo.tblDoctorSchedule.status, 
                      dbo.tblLocation.insidedr, dbo.tblDoctor.doctorcode, dbo.tblDoctorOffice.officecode, dbo.tblDoctorSchedule.locationcode, dbo.tblDoctor.Booking, 
                      dbo.tblDoctorSchedule.casenbr1, dbo.tblDoctorSchedule.casenbr2, dbo.tblDoctorSchedule.CaseNbr3, dbo.tblDoctorSchedule.starttime
FROM         dbo.tblDoctorSchedule INNER JOIN
...
SELECT     dbo.tblCase.casenbr, dbo.tblCase.claimnbr, dbo.tblExaminee.addr1 AS examineeaddr1, dbo.tblExaminee.addr2 AS examineeaddr2, 
                      ...
                      dbo.tblCase.InternalDueDate, dbo.tblLocation.ExtName AS LocationExtName, dbo.tblVenue.County AS Venue, dbo.tblOffice.description AS office, 
                      dbo.tblOffice.usdvarchar1 as OfficeUSDVarChar1, dbo.tblOffice.usdvarchar2 as OfficeUSDVarChar2
...
update tbldoctor set booking = 2
go
insert into tbluserfunction (functioncode, functiondesc) values('TripleBook', 'Appointments - TripleBook')
go
insert into tbluserfunction (functioncode, functiondesc)
 select 'GenCorrectedInv', 'Accounting - Generate Corrected Invoice'
 where not exists (select functionCode from tblUserFunction where functionCode='GenCorrectedInv')

go

update tblControl set DBVersion='1.06'
GO
