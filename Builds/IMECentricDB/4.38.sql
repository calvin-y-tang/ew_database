...
BEGIN TRANSACTION
GO
PRINT N'Starting rebuilding table [dbo].[tblDrDoNotUse]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_tblDrDoNotUse] (
    [Code]        INT            NOT NULL,
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_tblDrDoNotUse1] PRIMARY KEY CLUSTERED ([Code] ASC, [Type] ASC, [DoctorCode] ASC, [CaseType] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[tblDrDoNotUse])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_tblDrDoNotUse] ([Code], [Type], [DoctorCode], [DateAdded], [UserIDAdded], [Reason], [Note])
        SELECT   [Code],
                 [Type],
                 [DoctorCode],
                 [DateAdded],
                 ...
                 [Note]
        FROM     [dbo].[tblDrDoNotUse]
        ORDER BY [Code] ASC, [Type] ASC, [DoctorCode] ASC;
    END

DROP TABLE [dbo].[tblDrDoNotUse];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_tblDrDoNotUse]', N'tblDrDoNotUse';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_tblDrDoNotUse1]', N'PK_tblDrDoNotUse', N'OBJECT';
