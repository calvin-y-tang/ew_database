...
GO
PRINT N'Creating [dbo].[DF_tblDoctorBlockTimeDayActive]...';


GO
ALTER TABLE [dbo].[tblDoctorBlockTimeDay]
    ADD CONSTRAINT [DF_tblDoctorBlockTimeDayActive] DEFAULT ((0)) FOR [Active];
...
GO
PRINT N'Creating [dbo].[proc_Doctor_GetDaySheetData]...';
...
begin
	...
	;with daysheetdata as (
		...
		WHERE 					
			...
			--and tblCase.CaseNbr is not null --// need to comment this one out after testing as there are blocks that are scheduled but with no cases...using logic from vwRptDaySheet
	...
	,
	daysheetdatanew as (
		select distinct LocationCode from (
			select 
	...
	where 1=1 
		and (
			...
			(
			1 = (case when @alloffices = 1 then 0 else 1 end) 
			...
			(
			1 = (case when @alloffices = 1 then 1 else 0 end) 
			and LocationOffice = @office 
			and DoctorOffice = @office
			and 
			
			(
				(1 = (case when @usenewdata = 1 then 0 else 1 end) and (LocationCode in (select distinct LocationCode from daysheetdata x where OfficeCode = @office)))
				or
				(1 = (case when @usenewdata = 1 then 1 else 0 end) and (LocationCode in (select LocationCode from daysheetdatanew x where OfficeCode = @office)))
			)
