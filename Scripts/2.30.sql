...
ALTER TABLE [tblTranscription]
  ADD [UploadRptWithoutJob] BIT
GO

ALTER TABLE [tblTranscriptionJob]
  ADD [DictationDownloaded] BIT
GO

ALTER TABLE [tblTranscriptionJob]
  ADD [CoverLetterDownloaded] BIT
GO

ALTER TABLE [tblTranscriptionJob]
  ADD [ReportDownloaded] BIT
GO
...
ALTER TABLE [tblTranscriptionJob]
  ADD [OriginalRptFileName] VARCHAR(100)
GO

UPDATE tblTranscriptionJob SET
 DictationDownloaded=1,
 CoverLetterDownloaded=1,
 ReportDownloaded=1,
 OriginalRptFileName=ReportFile
GO

UPDATE tblTranscription SET
 UploadRptWithoutJob=0
GO

SET IDENTITY_INSERT tblTranscriptionStatus ON
INSERT INTO tblTranscriptionStatus
        ( TranscriptionStatusCode, Descrip )
VALUES  ( 45, 'New Report' )
SET IDENTITY_INSERT tblTranscriptionStatus OFF
GO

UPDATE tblTranscriptionJob SET Workflow=T.Workflow
 FROM tblTranscriptionJob AS J
 INNER JOIN tblTranscription AS T ON T.TransCode = J.TransCode
 WHERE J.Workflow IS NULL
GO


DROP PROCEDURE proc_RequestNextTranscriptionJob
GO
...
UPDATE tblControl SET DBVersion='2.30'
GO
